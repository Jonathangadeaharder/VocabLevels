...
# Single source of truth: extended_cognates.json (data, not code).
# work/clean_expansion.py proposes additions; merge them by hand.
def _load_extended_cognates() -> dict[str, set[str]]:
    raw = json.loads((ROOT / "extended_cognates.json").read_text(encoding="utf-8"))
    return {code: set(words) for code, words in raw.items()}


EXTENDED_COGNATE_ALLOWLIST: dict[str, set[str]] = _load_extended_cognates()
