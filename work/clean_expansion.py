...
and:
  ...
  - ungrounded_gloss_copy single-word lemmas -> emit as extra cognate
    allowlist (work/allowlist_extra.py), patch check_data_contract.py by hand
...
def main() -> None:
    ...
    for name, code in LANG_DIRS.items():
        ...
        if delete_keys or multis or prefixes:
            print(f"{code}: singles={len(singles)} multi_delete={len(multis)} "
                  f"prefix_delete={len(prefixes)} zh_script_junk={len(delete_keys) - len(multis) - len(prefixes)}")
        if apply and delete_keys:
            kept = [table[0]] + [r for r in table[1:]
                                 if (r[0], r[1], r[3]) not in delete_keys]
    ...
    if all_singles:
        out = ROOT / "work" / "allowlist_extra.py"
        lines = ["# Generated by work/clean_expansion.py: single-word",
        ...
        for code in sorted(all_singles):
            words = sorted({w.lower() for w in all_singles[code]})
            entries = ", ".join(repr(w) for w in words)
            lines.append(f"    '{code}': {{{entries}}},")
        lines.append("}")
        out.write_text("\n".join(lines) + "\n", encoding="utf-8")
        print(f"wrote {out} ({sum(len(v) for v in all_singles.values())} words)")
