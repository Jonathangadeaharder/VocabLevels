...
_STATE_DIRNAME = ".gemma_qa"
_LEDGER_FILENAME = "ledger.sqlite3"


def _ledger_for(root: Path) -> Ledger:
    return Ledger(root / _STATE_DIRNAME / _LEDGER_FILENAME)
...
def _run_normalize_english(args: argparse.Namespace) -> int:
...
def _run_handcraft_ready(args: argparse.Namespace) -> int:
...
def _format_event_line(item: dict[str, object]) -> str:
    kind = item.get("kind", "?")
    iso = item.get("iso", "")
    ...
    if item.get("batch_count") is not None:
        progress += f" batch={item.get('attempt')}/{item.get('batch_count')}"
    return (
        f"  {iso} {level} {kind}"
...
def _print_scale_progress(root: Path) -> None:
    ledger = _ledger_for(root)
    status = ledger.status()
    scale_state = ScaleState(root / _STATE_DIRNAME / "scale.sqlite3")
...
def _run_status(args: argparse.Namespace) -> int:
    _print_scale_progress(args.root)
    events_path = args.root / _STATE_DIRNAME / "events.jsonl"
    ...
    if recent:
        ...
        for item in recent:
            print(_format_event_line(item))
    else:
        print(f"events_path={events_path} recent=0")
    return 0


def _run_manual_review(args: argparse.Namespace) -> int:
...
def _run_seed_validated(args: argparse.Namespace) -> int:
...
def _run_refill(args: argparse.Namespace) -> int:
    client = _build_unified_client()
    ledger = _ledger_for(args.root)
    try:
...
def _run_scale(args: argparse.Namespace) -> int:
    ...
    result = run_scale(
        ScaleConfig(
            ...
            languages=tuple(get_language(language).code for language in args.languages),
...
def _run_cefr(args: argparse.Namespace) -> int:
    client = _build_unified_client()
    ledger = _ledger_for(args.root)
    try:
        output = run_cefr(
...
def _run_handcraft(args: argparse.Namespace) -> int:
    client = _build_unified_client()
    ledger = _ledger_for(args.vocab_root)
    try:
...
_COMMANDS = {
    "cefr": _run_cefr,
    "handcraft": _run_handcraft,
    "handcraft-ready": _run_handcraft_ready,
    "manual-review": _run_manual_review,
    "normalize-english": _run_normalize_english,
    "refill": _run_refill,
    "scale": _run_scale,
    "seed-validated": _run_seed_validated,
    "status": _run_status,
}
