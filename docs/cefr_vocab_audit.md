...
- `cleanup_inflections.py` only ever de-duplicates **English** same-level plurals (`if lang != "english": return 0`). No language-agnostic inflection cleanup exists for German/Swedish/Dutch/etc.
- `fix_pos_and_overflow.py --langs` **defaults to `["german", "arabic"]`** (`argparse` default, line 570). Stanza-based POS correction was never run against English, Spanish, French, Swedish, Dutch, or Chinese by default — this directly explains why those languages show the worst POS distributions below.
- `check_quality.py` has a **broken header check for the English/Chinese pivot languages**: it expects `[lemma_col, *schema_trans_cols, "POS"]`, but the actual on-disk CSVs use the harmonized dual-pivot shape `[<Lang>_Lemma, English_Lemma, Chinese_Lemma, POS]` (with the lemma column duplicated as its own "translation" for English, and duplicated for Chinese). Every row in every English and Chinese CSV therefore fails the header check and is **skipped entirely** — `check_quality.py` has never actually validated a single English or Chinese lemma cell. I validated those two languages manually (see below).
